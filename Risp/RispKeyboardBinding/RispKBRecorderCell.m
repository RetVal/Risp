...
#import <RispKeyboardBinding/RispKBRecorderCell.h>
#import <RispKeyboardBinding/RispKBRecorderControl.h>
#import <RispKeyboardBinding/RispKBKeyCodeTransformer.h>
#import <RispKeyboardBinding/RispKBValidator.h>

@interface RispKBRecorderCell (Private)
- (void)_privateInit;
...
@implementation RispKBRecorderCell
...
{
    RispKBRecorderCell *cell;
    cell = (RispKBRecorderCell *)[super copyWithZone: zone];
...
- (void)setStyle:(RispKBRecorderStyle)nStyle {
	switch (nStyle) {
		case RispKBGreyStyle:
			style = RispKBGreyStyle;
			break;
		case RispKBGradientBorderStyle:
		default:
			style = RispKBGradientBorderStyle;
			break;
	}
}
...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	if (style == RispKBGradientBorderStyle) {
	...
	// Draw gradient when in recording mode
		...
		// Draw snapback image
			NSImage *snapBackArrow = RispKBResIndImage(@"RispKBSnapback");	
	...
	// Draw border and remove badge if needed
		...
		// If key combination is set and valid, draw remove image
			...
			{
				NSString *removeImageName = [NSString stringWithFormat: @"RispKBRemoveShortcut%@", (mouseInsideTrackingArea ? (mouseDown ? @"Pressed" : @"Rollover") : (mouseDown ? @"Rollover" : @""))];
				NSImage *removeImage = RispKBResIndImage(removeImageName);
	...
	// Only the KeyCombo should be black and in a bigger font size
		...
		// Recording, but no modifier keys down
			if (![self _validModifierFlags: recordingFlags])
			{
				...
				{
				// Mouse over snapback
					displayString = RispKBLoc(@"Use old shortcut");
				}
				else
				{
				// Mouse elsewhere
					displayString = RispKBLoc(@"Type shortcut");
				}
			}
			else
			{
			// Display currently pressed modifier keys
				displayString = RispKBStringForCocoaModifierFlags( recordingFlags );
		...
		// Not recording...
			if ([self _isEmpty])
			{
				displayString = RispKBLoc(@"Click to record shortcut");
			}
			else
			{
...
//		NSLog(@"tp: %f; xanim: %f", transitionProgress, xanim);
			xanim = (RispKBAnimationEaseInOut(transitionProgress));
//		NSLog(@"tp: %f; xanim: %f", transitionProgress, xanim);
		}
...
//	if (isVaguelyRecording) 
		{
			NSRect snapBackRect = RispKBAnimationOffsetRect([self _snapbackRectForFrame: cellFrame],cellFrame);
...
//	}
	...
	// Calculate rect in which to draw the text in...
			NSRect textRect = RispKBAnimationOffsetRect(cellFrame,cellFrame);
...
{	
	RispKBRecorderControl *controlView = (RispKBRecorderControl *)[self controlView];
...
- (BOOL)trackMouse:(NSEvent *)theEvent inRect:(NSRect)cellFrame ofView:(RispKBRecorderControl *)controlView untilMouseUp:(BOOL)flag
{		
			...
			default: // NSLeftMouseUp
			{
				...
				if (mouseInsideTrackingArea)
				{
					...
					else
					{
						...
						[self setKeyCombo: RispKBMakeKeyCombo(ShortcutRecorderEmptyCode, ShortcutRecorderEmptyFlags)];
					}
				}
...
//						NSLog(@"calculated keychars: %@, ignoring: %@", RispKBStringForKeyCode(keyCombo.code), RispKBCharacterForKeyCodeAndCocoaFlags(keyCombo.code,keyCombo.flags));
...
- (NSString *)keyComboString
{
	...
	return [NSString stringWithFormat: @"%@%@",
        RispKBStringForCocoaModifierFlags( keyCombo.flags ),
        RispKBStringForKeyCode( keyCombo.code )];
}

- (NSString *)keyChars {
	if (!hasKeyChars) return RispKBStringForKeyCode(keyCombo.code);
	return keyChars;
}

- (NSString *)keyCharsIgnoringModifiers {
	if (!hasKeyChars) return RispKBCharacterForKeyCodeAndCocoaFlags(keyCombo.code,keyCombo.flags);
...
@implementation RispKBRecorderCell (Private)

- (void)_privateInit
{
    // init the validator object...
    validator = [[RispKBValidator alloc] initWithDelegate:self];
...
- (void)_startRecordingTransition {
	if ([self _effectiveIsAnimating]) {
		...
		[self performSelector:@selector(_transitionTick) withObject:nil afterDelay:(RispKBTransitionDuration/RispKBTransitionFrames)];
...
- (void)_transitionTick {
	transitionProgress += (1.0f/RispKBTransitionFrames);
...
- (void)_saveKeyCombo
{
	...
	if (defaultsKey != nil && [defaultsKey length])
	{
		...
		NSDictionary *defaultsValue = [NSDictionary dictionaryWithObjectsAndKeys:
			...
			[NSNumber numberWithUnsignedInteger:RispKBCocoaToCarbonFlags(keyCombo.flags)], @"modifiers", // carbon, for compatibility with RispKBKeyCombo
			nil];
...
- (void)_loadKeyCombo
{
	...
	if (defaultsKey != nil && [defaultsKey length])
	{
		...
		NSUInteger flags;
		if ((nil == [savedCombo valueForKey:@"modifierFlags"]) && (nil != [savedCombo valueForKey:@"modifiers"])) { // carbon, for compatibility with RispKBKeyCombo
			flags = RispKBCarbonToCocoaFlags([[savedCombo valueForKey: @"modifiers"] unsignedIntegerValue]);
		} else { // cocoa
...
{	
	...
	NSImage *removeImage = RispKBResIndImage(@"RispKBRemoveShortcut");
...
//	if (!isRecording) return NSZeroRect;
	...
	NSRect snapbackRect;
	NSImage *snapbackImage = RispKBResIndImage(@"RispKBSnapback");
...
- (BOOL)_isEmpty
{
	return ( ![self _validModifierFlags: keyCombo.flags] || !RispKBStringForKeyCode( keyCombo.code ) );
}

#pragma mark *** Delegate pass-through ***

- (BOOL) shortcutValidator:(RispKBValidator *)validator isKeyCode:(NSInteger)keyCode andFlagsTaken:(NSUInteger)flags reason:(NSString **)aReason;
{
