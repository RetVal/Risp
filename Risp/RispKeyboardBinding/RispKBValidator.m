...
#import "RispKBValidator.h"
#import "RispKBCommon.h"

@implementation RispKBValidator
...
- (BOOL) isKeyCode:(NSInteger)keyCode andFlagsTaken:(NSUInteger)flags error:(NSError **)error;
{
	...
	if ( delegate )
	{
		...
		if ( [delegate shortcutValidator:self 
                               isKeyCode:keyCode 
                           andFlagsTaken:RispKBCarbonToCocoaFlags( flags )
                                  reason:&delegateReason])
		{
            if ( error )
            {
                NSString *description = [NSString stringWithFormat: 
                    RispKBLoc(@"The key combination %@ can't be used!"), 
                    RispKBStringForCarbonModifierFlagsAndKeyCode( flags, keyCode )];
                NSString *recoverySuggestion = [NSString stringWithFormat: 
                    RispKBLoc(@"The key combination \"%@\" can't be used because %@."), 
                    RispKBReadableStringForCarbonModifierFlagsAndKeyCode( flags, keyCode ),
	...
	while (( globalHotKeyInfoDictionary = [globalHotKeysEnumerator nextObject] ))
	{
        ...
        NSString *localKeyString = RispKBStringForKeyCode( keyCode );
             ...
             && ( globalCtrlMod == localCtrlMod ) )
        {
            if ( error )
            {
                NSString *description = [NSString stringWithFormat: 
                    ...
                    RispKBStringForCarbonModifierFlagsAndKeyCode( flags, keyCode )];
                NSString *recoverySuggestion = [NSString stringWithFormat: 
                    RispKBLoc(@"The key combination \"%@\" can't be used because it's already used by a system-wide keyboard shortcut. (If you really want to use this key combination, most shortcuts can be changed in the Keyboard & Mouse panel in System Preferences.)"), 
                    RispKBReadableStringForCarbonModifierFlagsAndKeyCode( flags, keyCode )];
				NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
										  description, NSLocalizedDescriptionKey,
										  recoverySuggestion, NSLocalizedRecoverySuggestionErrorKey,
                 ...
                 && ( menuItemCtrlMod == localCtrlMod ) )
			{
                if ( error )
                {
                    NSString *description = [NSString stringWithFormat: 
                        ...
                        RispKBStringForCarbonModifierFlagsAndKeyCode( flags, keyCode )];
                    NSString *recoverySuggestion = [NSString stringWithFormat: 
                        RispKBLoc(@"The key combination \"%@\" can't be used because it's already used by the menu item \"%@\"."), 
                        RispKBReadableStringForCocoaModifierFlagsAndKeyCode( menuItemModifierFlags, keyCode ),
                        [menuItem title]];
                    NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:
											  description, NSLocalizedDescriptionKey,
											  recoverySuggestion, NSLocalizedRecoverySuggestionErrorKey,
...
@implementation NSObject( RispKBValidation )
