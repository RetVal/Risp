...
#import "RispRenderFoundationAutoCompleteDelegate.h"

// class extension
@interface RispRenderFoundationTextView()
...
@implementation RispRenderFoundationTextView
...
- (id)initWithFrame:(NSRect)frame
{
	if ((self = [super initWithFrame:frame])) {
		RispRenderFoundationLayoutManager *layoutManager = [[RispRenderFoundationLayoutManager alloc] init];
...
- (void)setDefaults
{
	...
	[self setContinuousSpellCheckingEnabled:[[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutoSpellCheck] boolValue]];
	[self setGrammarCheckingEnabled:[[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutoGrammarCheck] boolValue]];
	
	[self setSmartInsertDeleteEnabled:[[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsSmartInsertDelete] boolValue]];
	[self setAutomaticLinkDetectionEnabled:[[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutomaticLinkDetection] boolValue]];
	[self setAutomaticQuoteSubstitutionEnabled:[[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutomaticQuoteSubstitution] boolValue]];
	...
	[defaultsController addObserver:self forKeyPath:@"values.RFFTextFont" options:NSKeyValueObservingOptionNew context:@"TextFontChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFTextColourWell" options:NSKeyValueObservingOptionNew context:@"TextColourChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFBackgroundColourWell" options:NSKeyValueObservingOptionNew context:@"BackgroundColourChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFSmartInsertDelete" options:NSKeyValueObservingOptionNew context:@"SmartInsertDeleteChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFTabWidth" options:NSKeyValueObservingOptionNew context:@"TabWidthChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFShowPageGuide" options:NSKeyValueObservingOptionNew context:@"PageGuideChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFShowPageGuideAtColumn" options:NSKeyValueObservingOptionNew context:@"PageGuideChanged"];
	[defaultsController addObserver:self forKeyPath:@"values.RFFSmartInsertDelete" options:NSKeyValueObservingOptionNew context:@"SmartInsertDeleteChanged"];
	
	lineHeight = [[[self textContainer] layoutManager] defaultLineHeightForFont:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextFont]]];
}
...
{
	[self setFont:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextFont]]];
	[self setTextColor:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextColourWell]]];
	[self setInsertionPointColor:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextColourWell]]];
	[self setBackgroundColor:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsBackgroundColourWell]]];
}
...
-(void)paste:(id)sender
{
    ...
    NSNotification *note = [NSNotification notificationWithName:@"RispRenderFoundationTextDidPasteNotification" object:self userInfo:info];
    [[NSNotificationCenter defaultCenter] postNotification:note];
    
    // inform delegate of Fragaria paste
    if ([self.delegate respondsToSelector:@selector(RispRenderFoundationTextDidPaste:)]) {
        [(id)self.delegate RispRenderFoundationTextDidPaste:note];
    }
}
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
	...
	/*
	 ...
	 NSEnumerator *collectionEnumerator = [[RispRenderFoundationBasic fetchAll:@"SnippetCollectionSortKeyName"] reverseObjectEnumerator];
	 ...
	 [subMenuItem setTarget:[RispRenderFoundationToolsMenuController sharedInstance]];			
...
{	
	...
	if (shouldShiftText) {
		[[RispRenderFoundationTextMenuController sharedInstance] shiftRightAction:nil];
	} else if ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsIndentWithSpaces] boolValue] == YES) {
		NSMutableString *spacesString = [NSMutableString string];
		NSInteger numberOfSpacesPerTab = [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTabWidth] integerValue];
		if ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsUseTabStops] boolValue] == YES) {
...
- (void)setTabWidth
{
	...
	NSInteger numberOfSpaces = [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTabWidth] integerValue];
	while (numberOfSpaces--) {
		[sizeString appendString:@" "];
	}
	NSDictionary *sizeAttribute = [[NSDictionary alloc] initWithObjectsAndKeys:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextFont]], NSFontAttributeName, nil];
...
- (void)setPageGuideValues
{
	...
	pageGuideX = (sizeOfCharacter * ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsShowPageGuideAtColumn] integerValue] + 1)) - 1.5f; // -1.5 to put it between the two characters and draw only on one pixel and not two (as the system draws it in a special way), and that's also why the width above is set to zero 
	
	NSColor *color = [NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextColourWell]];
	...
	showPageGuide = [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsShowPageGuide] boolValue];
...
{
	if ([aString isEqualToString:@"}"] && [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsIndentNewLinesAutomatically] boolValue] == YES && [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutomaticallyIndentBraces] boolValue] == YES) {
	...
	} else if ([aString isEqualToString:@"("] && [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutoInsertAClosingParenthesis] boolValue] == YES) {
		[super insertText:aString];
	...
	} else if ([aString isEqualToString:@"{"] && [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutoInsertAClosingBrace] boolValue] == YES) {
		[super insertText:aString];
		NSRange selectedRange = [self selectedRange];
		if ([self shouldChangeTextInRange:selectedRange replacementString:@"}"]) {
			[self replaceCharactersInRange:selectedRange withString:@"}"];
...
- (void)insertNewline:(id)sender
{
	...
	if ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsIndentNewLinesAutomatically] boolValue] == YES) {
		...
		if ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsAutomaticallyIndentBraces] boolValue] == YES) {
...
{
	NSColor *textColour = [[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextColourWell]] colorUsingColorSpaceName:NSCalibratedWhiteColorSpace];
	...
	if (textColour != nil && [textColour whiteComponent] < 0.01 && [textColour alphaComponent] > 0.990) { // Keep the original cursor if it's black
		...
		[(NSColor *)[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextColourWell]] set];
...
#pragma unused(index, charRange)
    ...
    id<RispRenderFoundationAutoCompleteDelegate> completeHandler = [fragaria.docSpec valueForKey:RispRenderFoundationFOAutoCompleteDelegate];
...
{
	[RispRenderFoundation setCurrentInstance:self.fragaria];
