...
#import "RispRenderFoundationLayoutManager.h"

typedef enum {
    kTabLine = 0,
    kSpaceLine = 1,
    kNewLineLine = 2
} RispRenderFoundationLineCacheIndex;

@interface RispRenderFoundationLayoutManager()
- (void)resetAttributesAndGlyphs;
@end

@implementation RispRenderFoundationLayoutManager
...
- (id)init
{
	...
	if (self) {
		...
		[self setShowInvisibleCharacters:[[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsShowInvisibleCharacters] boolValue]];
		...
		[defaultsController addObserver:self forKeyPath:@"values.RFFTextFont" options:NSKeyValueObservingOptionNew context:@"FontOrColourValueChanged"];
		[defaultsController addObserver:self forKeyPath:@"values.RFFInvisibleCharactersColourWell" options:NSKeyValueObservingOptionNew context:@"FontOrColourValueChanged"];
        
        // invisible characters preference
        [defaultsController addObserver:self forKeyPath:@"values.RFFShowInvisibleCharacters" options:NSKeyValueObservingOptionNew context:@"InvisibleCharacterValueChanged"];
        
        
        // assign our custom glyph generator
        if (useGlyphSubstitutionForInvisibleGlyphs) {
            [self setGlyphGenerator:[[RispRenderFoundationGlyphGenerator alloc] init]];
        }
...
//#define RispRenderFoundation_PROFILE_DRAW
    
#ifdef RispRenderFoundation_PROFILE_DRAW
...
- (void)resetAttributesAndGlyphs
{
    ...
    defAttributes = [[NSDictionary alloc] initWithObjectsAndKeys:
                  [NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextFont]], NSFontAttributeName, [NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsInvisibleCharactersColourWell]], NSForegroundColorAttributeName, nil];
    ...
    if (drawInvisibleGlyphsUsingCoreText) {
        ...
        CTLineRef textLine = CFAutorelease(CTLineCreateWithAttributedString((CFAttributedStringRef)attrString));
        ...
        textLine = CFAutorelease(CTLineCreateWithAttributedString((CFAttributedStringRef)attrString));
