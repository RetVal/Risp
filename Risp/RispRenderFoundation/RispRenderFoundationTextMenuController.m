...
// class extension
@interface RispRenderFoundationTextMenuController()
...
@implementation RispRenderFoundationTextMenuController
 ...
 */
+ (RispRenderFoundationTextMenuController *)sharedInstance
{ 
...
- (void)setEdited:(BOOL)aBool
{
	if ([[RispRenderFoundationCurrentDocument valueForKey:RispRenderFoundationFOIsEdited] boolValue] != aBool) {			
		[[RispRenderFoundation currentInstance] setObject:[NSNumber numberWithBool:aBool] forKey:RispRenderFoundationFOIsEdited];
	}
}
...
{
	[RispRenderFoundationBasic removeAllItemsFromMenu:textEncodingMenu];
	[RispRenderFoundationBasic removeAllItemsFromMenu:reloadTextWithEncodingMenu];
	
	NSArray *encodingsArray = [RispRenderFoundationBasic fetchAll:@"EncodingSortKeyName"];
...
{
	NSArray *syntaxDefinitions = [RispRenderFoundationBasic fetchAll:@"SyntaxDefinitionSortKeySortOrder"];
...
- (BOOL)validateMenuItem:(NSMenuItem *)anItem
{
	...
	id responder = [RispRenderFoundationCurrentWindow firstResponder];
	if (![responder isKindOfClass:[RispRenderFoundationTextView class]]) {
		return NO;
	}
	...
	if (action == @selector(removeNeedlessWhitespaceAction:) ||
		...
		if ([RispRenderFoundationCurrentTextView selectedRange].length < 1) {
			enableMenuItem = NO;
		}
	}
    // Items which should be active if nothing is selected
    else if (action == @selector(toggleBreakpointAction:)) {
        if ([RispRenderFoundationCurrentTextView selectedRange].length > 0) {
            enableMenuItem = NO;
        }
    }
    // Comment Or Uncomment
    else if (action == @selector(commentOrUncommentAction:) ) {
		if ([[[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOSyntaxColouring] valueForKey:@"firstSingleLineComment"] isEqualToString:@""]) {
			enableMenuItem = NO;
		}
	} 
...
{	
	...
	id document = RispRenderFoundationCurrentDocument;
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:@"RispRenderFoundationTextEncodingChanged" object:[RispRenderFoundation currentInstance] userInfo:nil];
}
...
#pragma unused(sender)
	...
	NSTextView *textView = RispRenderFoundationCurrentTextView;
	
	NSString *completeString = [textView string];
	if ([completeString length] < 1) {
		return;
	}
	[[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOSyntaxColouring] setReactToChanges:NO];
	NSRange selectedRange;
	
	NSArray *array = [RispRenderFoundationCurrentTextView selectedRanges];
	...
	for (id item in array) {
		...
		NSInteger numberOfSpacesPerTab = [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsIndentWidth] integerValue];
		...
		for (idx = 0; idx < numberOfLines; idx++) {
			...
			if ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsUseTabStops] boolValue] == YES && [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsIndentWithSpaces] boolValue] == YES) {
	...
	[[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOSyntaxColouring] setReactToChanges:YES];
	
	[[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOSyntaxColouring] pageRecolour];
	
	if (sumOfAllCharactersRemoved == 0) {
		NSBeep();
	} else {
		if ([[RispRenderFoundationCurrentDocument valueForKey:RispRenderFoundationFOIsEdited] boolValue] == NO) {
			[self setEdited: YES];
		}
		[[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOLineNumbers] updateLineNumbersCheckWidth:NO recolour:NO];
	}
...
- (IBAction)shiftRightAction:(id)sender
{
	...
	if ([[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsIndentWithSpaces] boolValue] == YES) {
...
- (IBAction)toLowercaseAction:(id)sender
{
	...
	for (id item in array) {
		...
		NSString *originalString = [RispRenderFoundationCurrentText substringWithRange:selectedRange];
...
- (IBAction)entabAction:(id)sender
{
	...
	[RispRenderFoundationCurrentExtraInterfaceController displayEntab];
}
...
- (IBAction)detabAction:(id)sender
{
	...
	[RispRenderFoundationCurrentExtraInterfaceController displayDetab];
}
...
{	
	...
	NSInteger numberOfSpaces = [[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsSpacesPerTabEntabDetab] integerValue];
...
#pragma unused(sender)
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:@"RispRenderFoundationReloadText" object:[RispRenderFoundation currentInstance] userInfo:nil];
}
...
- (IBAction)goToLineAction:(id)sender
{
	...
	[RispRenderFoundationCurrentExtraInterfaceController displayGoToLine];
}
...
- (void)performGoToLine:(NSInteger)lineToGoTo
{
	...
	NSInteger idx;
	NSString *completeString = RispRenderFoundationCurrentText;
	...
	[RispRenderFoundationCurrentTextView setSelectedRange:[completeString lineRangeForRange:NSMakeRange(idx, 0)]];
	[RispRenderFoundationCurrentTextView scrollRangeToVisible:[completeString lineRangeForRange:NSMakeRange(idx, 0)]];
}
...
- (IBAction)commentOrUncommentAction:(id)sender
{
	...
	NSTextView *textView = RispRenderFoundationCurrentTextView;
	NSString *completeString = [textView string];
	NSString *commentString = [[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOSyntaxColouring] valueForKey:@"firstSingleLineComment"];
...
- (IBAction)removeLineEndingsAction:(id)sender
{
	...
	for (id item in array) {
		...
		NSString *stringWithNoLineEndings = [RispRenderFoundationText removeAllLineEndingsInString:stringToRemoveLineEndingsFrom];
	...
	[self setEdited:YES];	
	[[RispRenderFoundationCurrentDocument valueForKey:ro_MGSFOLineNumbers] updateLineNumbersCheckWidth:YES recolour:YES];
...
- (IBAction)changeLineEndingsAction:(id)sender
{
	...
	NSRange selectedRange = [textView selectedRange];
	NSString *text = [textView string];
	NSString *convertedString = [RispRenderFoundationText convertLineEndings:text inDocument:document];
...
{
	id document = RispRenderFoundationCurrentDocument;
	[document setValue:[sender title] forKey:RispRenderFoundationFOSyntaxDefinitionName];
	...
	[[document valueForKey:ro_MGSFOSyntaxColouring] pageRecolour];
	[[NSNotificationCenter defaultCenter] postNotificationName:@"RispRenderFoundationSyntaxDefinitionChanged" object:[RispRenderFoundation currentInstance] userInfo:nil];
