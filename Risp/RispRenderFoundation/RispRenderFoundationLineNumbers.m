...
@interface RispRenderFoundationLineNumbers()
...
@implementation RispRenderFoundationLineNumbers
...
- (id)initWithDocument:(id)theDocument
{
	if ((self = [super init])) {
		...
		self.attributes = [[NSDictionary alloc] initWithObjectsAndKeys:[NSUnarchiver unarchiveObjectWithData:[RispRenderFoundationDefaults valueForKey:RispRenderFoundationPrefsTextFont]], NSFontAttributeName, nil];
...
- (void)updateLineNumbersForClipView:(NSClipView *)clipView checkWidth:(BOOL)checkWidth recolour:(BOOL)recolour
{
    ...
    @try {
        RispRenderFoundationTextView *textView = [clipView documentView];
        
        if ([[document valueForKey:RispRenderFoundationFOShowLineNumberGutter] boolValue] == NO || textView == nil) {
        ...
        id breakpointDelegate = [[RispRenderFoundation currentInstance] objectForKey:RispRenderFoundationFOBreakpointDelegate];
        ...
        if (checkWidth == YES) {
            ...
            if (widthOfStringInGutter > ([[document valueForKey:RispRenderFoundationFOGutterWidth] integerValue] - 14)) { // Check if the gutterTextView has to be resized
                [document setValue:[NSNumber numberWithInteger:widthOfStringInGutter + 20] forKey:RispRenderFoundationFOGutterWidth]; // Make it bigger than need be so it doesn't have to resized soon again
                if ([[document valueForKey:RispRenderFoundationFOShowLineNumberGutter] boolValue] == YES) {
                    gutterWidth = [[document valueForKey:RispRenderFoundationFOGutterWidth] integerValue];
                } else {
                    gutterWidth = 0;
                }
                ...
                [gutterScrollView setFrame:NSMakeRect(0, 0, [[document valueForKey:RispRenderFoundationFOGutterWidth] integerValue], currentViewBounds.size.height)];
            }
        }
