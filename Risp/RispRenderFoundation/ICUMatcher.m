...
-(NSString *)groupAtIndex:(unsigned)groupIndex {
	...
	while(YES) { 
		UErrorCode status = 0;
		UChar *dest = (UChar *)NSZoneCalloc(NSDefaultMallocZone(), groupSize, sizeof(UChar));
		int32_t buffSize = uregex_group(re, groupIndex, dest, (int32_t)groupSize, &status);

		if(U_BUFFER_OVERFLOW_ERROR == status) {
			groupSize *= 2;
			NSZoneFree(NSDefaultMallocZone(), dest);
			continue;
		}
...
-(NSString *)performReplacementWithString:(NSString *)aReplacementString replaceAll:(BOOL)replacingAll {
	...
	while(!replacementCompleted) {
		...
		if(!destString) // attempts to increase buffer happen on failure below
			destString = NSZoneCalloc(NSDefaultMallocZone(), destStringBufferSize, sizeof(UChar));
		...
		if(status == U_BUFFER_OVERFLOW_ERROR) {
			...
			destString = NSZoneRealloc(NSDefaultMallocZone(), destString, destStringBufferSize*sizeof(UChar));
			
			if(destString == NULL) {
				NSZoneFree(NSDefaultMallocZone(), prevString);
				[NSException raise:@"Find Exception"
							format:@"Could not allocate memory for replacement string"];
			}
		} else if(U_FAILURE(status)) {
			NSZoneFree(NSDefaultMallocZone(), destString);
			[NSException raise:@"Find Exception"
